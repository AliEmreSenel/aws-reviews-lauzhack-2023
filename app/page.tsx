...
export default function Home() {
    ...
    useEffect(() => {
        if (PRODUCT) {
            ...
            const filteredReviewProductWithoutEmptyClustersAndMergedClustersWithSameInterpret = filteredReviewProductWithoutEmptyClusters.map((category) => ({ ...category, clusters: category.clusters.reduce((acc, cluster) => { const index = acc.findIndex((c) => c.clusterInterpret === cluster.clusterInterpret); if (index === -1) { return [...acc, cluster]; } else { return [...acc.slice(0, index), { ...acc[index], reviews: [...acc[index].reviews, ...cluster.reviews] }, ...acc.slice(index + 1)]; } }, []) }));
            const filteredReviewProductWithoutEmptyCategories = filteredReviewProductWithoutEmptyClustersAndMergedClustersWithSameInterpret.filter((category) => category.clusters.length > 0);
